...
export default function BHRPreview() {
  ...
  const nav = (r: string) =>
    route === r
      ? "text-[#0e64ff] font-semibold"
      : "text-slate-600 hover:text-[#0e64ff]";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f6f9ff] via-[#eef2ff] to-white text-slate-900 pt-24">
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            ...
            <button className={nav("why")} onClick={() => setRoute("why")}> {t.nav.why} </button>
            <button className={nav("resources")} onClick={() => setRoute("resources")}> {t.nav.resources} </button>
            <button className={nav("jobs")} onClick={() => setRoute("jobs")}>
              {t.nav.jobs}
            </button>
            <button className={nav("quote")} onClick={() => setRoute("quote")}> {t.nav.quote} </button>
          ...
          <div className="flex items-center gap-3 text-sm">
            <a
              href={`tel:${t.header.phone.split(" ").join("")}`}
      ...
      {route === "home" && (
        <Home t={t} goQuote={() => setRoute("quote")} goServices={() => setRoute("services")} />
      )}
...
function Home({ t, goQuote, goServices }: any) {
  return (
      ...
      <div className="grid lg:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <div className="mt-8 grid sm:grid-cols-3 gap-4">
            {t.home.quick.map((x: any, i: number) => (
              <div key={i} className={UI.card}>
                <strong className="block text-slate-900">{x.t}</strong>
                <span className="text-slate-600">{x.d}</span>
              </div>
            ))}
          </div>
        </div>
        <div className={UI.card}>
          <p className="text-sm text-slate-600">{t.hero.security}</p>
          <dl className="mt-4 grid grid-cols-3 gap-4 text-sm">
            <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
              <dt className="text-slate-500">{t.hero.kpis[0].label}</dt>
              <dd className="mt-1 text-2xl font-semibold">120+</dd>
            </div>
            <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
              <dt className="text-slate-500">{t.hero.kpis[1].label}</dt>
              <dd className="mt-1 text-2xl font-semibold">&lt; 24h</dd>
            </div>
            <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
              <dt className="text-slate-500">{t.hero.kpis[2].label}</dt>
              <dd className="mt-1 text-2xl font-semibold">4.9/5</dd>
            </div>
          </dl>
        </div>
      </div>
  ...
  return (
    <section className={UI.section}>
      ...
      <div className="grid md:grid-cols-4 gap-3 mt-4">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t.jobs.filters.search} className={UI.input} />
  ...
  return (
    <section className={UI.section + " space-y-8"}>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[...t.resources.guides, ...t.resources.faq, ...t.resources.webinars, ...t.resources.models].map((r: any, i: number) => (
...
function QuotePage({ lang, t }: any) {
  ...
  useMemo(
    () =>
      "Je souhaite effectuer une simulation de devis en ligne et j’autorise B‑HR à utiliser mes données pour ce calcul ; le montant estimatif me sera envoyé directement par e‑mail.",
    []
  );

  const perEmployee = (n: number) => {
  ...
  const approxFromRange = (r: string) => ({
    "1–5": 5,
    "6–10": 10,
    "11–20": 20,
    "21–50": 50,
  ...
  const effective = typeof headcount === "number" && headcount > 0 ? headcount : approxFromRange(range);
  const unit = perEmployee(effective!);
  const estimate = unit && effective && effective <= 50 ? unit * effective : undefined;
  ...
  const onSubmit = (e: any) => {
    ...
    fetch("/api/contact", {
      ...
      body: JSON.stringify({
        ...d,
        exactEmployees: headcount || null,
        estimateTotal: estimate ?? null,
        lang,
        intent: "online_quote",
      }),
    })
  ...
  return (
    <section className={UI.section.replace("py-16", "py-12") + " max-w-3xl"}>
      ...
      <p className="text-slate-600">
        Remplissez le formulaire : vous recevez le montant estimatif par e‑mail et réservez un rendez‑vous en ligne ou par téléphone.
      </p>
      <form onSubmit={onSubmit} className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="employees" required defaultValue="" onChange={(e) => setRange(e.target.value)} className={UI.input + " bg-white"}>
            <option value="" disabled>
        ...
        <div>
          <label className="block text-sm text-slate-600 mb-1">Nombre exact (optionnel)</label>
          <input type="number" min={1} max={50} inputMode="numeric" name="exactEmployees" onChange={(e) => setHeadcount(e.target.value ? Number(e.target.value) : "")} className={UI.input} placeholder="1–50" />
        </div>
        <div className="sm:col-span-2">
          <fieldset className="rounded-2xl border border-slate-200/70 bg-white/70 p-4">
            <legend className="text-sm text-slate-600">Êtes‑vous affilié à un autre secrétariat social ?</legend>
            <div className="mt-2 flex flex-wrap gap-6">
              <label className="inline-flex items-center gap-2 text-sm">
                <input type="radio" name="affiliated" value="yes" required className="accent-[#0e64ff]" onChange={() => setAffiliated("yes")} /> Oui
              </label>
              <label className="inline-flex items-center gap-2 text-sm">
                <input type="radio" name="affiliated" value="no" className="accent-[#0e64ff]" onChange={() => setAffiliated("no")} /> Non
              </label>
            </div>
            {affiliated === "yes" && (
              <div className="mt-4">
        ...
        <div className="sm:col-span-2 rounded-2xl border border-slate-200/70 bg-white/70 backdrop-blur p-4">
          <h4 className="font-medium">Tarification communiquée en privé</h4>
          <p className="mt-2 text-sm text-slate-700">Vous recevrez une estimation personnalisée par e‑mail.</p>
          <div className="mt-3 text-sm font-semibold">
            Estimation totale: {estimate ? `€${estimate}` : "—"}
          </div>
        </div>
        ...
        <div className="sm:col-span-2 flex justify-end gap-3">
          <button type="submit" className={UI.btnPrimary}>Envoyer</button>
        </div>
      </form>
    </section>
  );
